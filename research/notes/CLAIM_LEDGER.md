...
| ANT-CLM-006 | Claim | Protocol `ANT-PROT-FEAS-001` prospectively freezes separate design-traceability, synthetic technical, real-model technical, and within-person record-feasibility stages, together with conditions, measures, missingness, progression, safety, and deviation rules. | Frozen protocol version 1.0.0, content lock, empty deviation log, schema mappings, and issue #40 manuscript review. | Methods; Results; Appendix; Limitations. | Accepted protocol-artifact claim. Collection authority remains false; do not imply execution, participants, ethics approval, safety, efficacy, or observed outcomes. |
| ANT-CLM-007 | Claim | Results reporting contract `ANT-REPORT-RESULTS-001` binds every future result slot to a named source record, protocol version, analysis pointer, evidence class, and promotion review, plus a governed visual projection where one is planned, while preserving non-positive and unavailable states. | Versioned reporting contract, deterministic empty-state tables, manuscript Results section, and issue #41 tests. | Results; Appendix; Availability. | Accepted reporting-architecture claim. The contract does not create evidence, validate an analysis, authorize collection, or support technical, subjective, safety, mechanism, or efficacy findings. |
