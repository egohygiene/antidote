...
The generated [`research-shelf.tex`](research-shelf.tex) appendix projection
lists verified bibliography entries that are not cited by the current
manuscript. It preserves additional reading without inflating citation counts
or implying that every catalog entry supports a claim. Refresh or verify it
with:

```sh
python3 scripts/generate_research_shelf.py --write
...
All governed visuals already occupy their intended manuscript locations.
Deterministic layout frames generated by
`scripts/generate_skeleton_visuals.py` remain visibly provisional and contain
no result values. Issue #46 owns their replacement with final, reviewed assets;
the manifest state must change with the asset.
