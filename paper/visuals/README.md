...
Issue #46 advances the visual system from layout planning to publication
production. Seventeen active records are final and referenced exactly once.
Eight exact diagrams are generated by
`scripts/generate_publication_figures.py`; three non-result tables are generated
by `scripts/generate_visual_tables.py`; the comparator, Results projections,
and risk table retain their existing deterministic generators. `ANT-FIG-002`
is a separately authored, reviewed deterministic SVG.

`ANT-FIG-010` and `ANT-FIG-011` are evidence-contingent records preserved with
`retired` status. They are absent from the current manuscript because neither
the qualifying T0 interface-capture package nor the qualifying T1 benchmark
package exists. Their source specifications define fail-closed reactivation;
retirement is not evidence that those evaluations succeeded or failed.

Regenerate owned assets only through their source scripts:

```sh
python3 scripts/generate_publication_figures.py --write
python3 scripts/generate_visual_tables.py --write
python3 scripts/generate_comparator_table.py --write
python3 scripts/generate_results_reporting.py --write
python3 scripts/generate_risk_table.py --write
```
...
other than `final`. Planned or retired visuals may not be referenced, and
active visuals may not be missing or unreferenced.
