...
def author_lines(authors: list[dict]) -> tuple[str, str, str]:
    ...
    for author in authors:
        ...
        rendered_authors.append(
            name
            + (rf" \textnormal{{\small ({'; '.join(details)})}}" if details else "")
        )
...
def write_generated_metadata(
    ...
    commands = {
        ...
        "BeaconPaperRevisionShort": tex_escape(revision[:12]),
        "BeaconContributorStatement": credits,
    }
    lines = ["% Generated by Beacon. Edit beacon-project.toml, not this file."]
    lines.extend(
        rf"\renewcommand{{\{name}}}{{{value}}}" for name, value in commands.items()
    )
    lines.append(
...
def build_web(
    work: Path, output: Path, config: dict, theme: dict, revision: str
) -> None:
    paper = config["paper"]
...
def main() -> int:
    ...
    revision = git_revision(
        project, config["provenance"].get("source_revision", "working-tree")
    )
    ...
    write_generated_metadata(
        work / "beacon-generated-metadata.tex", config, revision, manifest["version"]
    )
    ...
    build_web(work, output, config, theme, revision)
